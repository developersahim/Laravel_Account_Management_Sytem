...
<div class="space-y-6 max-w-xl mx-auto">
    ...
    <form action="{{ route('incomes.store') }}" method="POST" class="space-y-4 bg-white shadow rounded-lg p-6">
        ...
        <div>
            ...
            <input type="text" name="payer" value="{{ old('payer') }}"
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                      class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description') }}</textarea>
            @error('description') 
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p> 
            @enderror
        </div>
